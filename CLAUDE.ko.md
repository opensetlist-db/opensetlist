...
✅ Song          — 곡의 모든 공연
✅ Event         — 특정 콘서트 날짜
✅ EventSeries   — 전체 투어/페스티벌 시리즈
✅ Artist        — 아티스트 토론 게시판
✅ Group         — 프랜차이즈/레이블 게시판 (hasBoard=true만)
❌ StageIdentity — 제외 (Artist 게시판으로 커버)
❌ RealPerson    — 제외 (실존 인물 게시판은 안전 문제)
❌ Album         — 제외 (Song 페이지에서 논의)
```

### 롤업 앤세스트리 (Roll-up Ancestry)
가장 구체적인 레벨에 달린 댓글이 상위 레벨 게시판 모두에 자동으로 표시됨.
모든 앤세스트리 필드는 작성 시 서버에서 계산되며 이후 불변.
클라이언트는 앤세스트리 필드를 절대 전송하지 않음 — 서버가 DB에서 직접 계산.

```
6개 롤업 배열 (PostgreSQL 네이티브 배열, GIN 인덱스):
  rollupSongIds[]        셋리스트 아이템의 곡들 (메들리: 여러 곡)
  rollupEventIds[]       리프 이벤트 + 모든 조상 이벤트 (깊이 제한 없음)
  rollupEventSeriesIds[] 직접 시리즈 + 모든 조상 시리즈 (깊이 제한 없음)
  rollupArtistIds[]      직접 아티스트 + 모든 상위 아티스트 (깊이 제한 없음)
  rollupGroupIds[]       hasBoard=true 그룹만 — 관리자 제어
  rollupCategories[]     ["anime"] | ["kpop"] | ["jpop"] 등
```

예: 4th Live 고베 Day 2의 "Hanamusubi" SetlistItem 댓글이 표시되는 게시판:
```
SetlistItem:    Hanamusubi @ 고베 Day 2
Song:           Hanamusubi
Event:          고베 Day 2
Event:          고베 레그 (부모 이벤트)
...
Artist:         蓮ノ空 (상위 아티스트)
Group:          Love Live! 시리즈 (hasBoard=true)
Category:       anime
```

### 게시판 거버넌스
- `Group.hasBoard` — 관리자 전용 토글. 크라우드소싱 그룹 추가로 인한 무제한 게시판 생성 방지.
- `rollupGroupIds`는 작성 시점에 `hasBoard=true`인 그룹만 포함.

### 댓글 무결성 규칙
- **타겟 필드 불변** — 유저가 댓글을 다른 엔티티로 이동 불가.
- **앤세스트리 불변** — 작성 시 한 번만 계산, 이후 변경 불가 (관리자 재인덱스 제외).
- **내용 수정 가능** — 소유자만 가능, 모든 변경은 `CommentEdit`에 기록.
- **소프트 삭제만** — `isDeleted=true`, 내용 "[deleted]"로 교체, 행은 유지.

### 라이브 이벤트 지원
- `Event.status = "ongoing"` → UI 실시간 모드 활성화.
- `SetlistItem.status = "live"` → 현재 공연 중인 곡 표시.
- `SetlistItem.status = "rumoured"` → 이벤트 전 팬 예측.
- `Comment.mentionedSongId` → SetlistItem 없이 이벤트 댓글에서 곡 태그.
...
## 핵심 DB 스키마 (Prisma) — 최종 Core v9 + Community v4

전체 스키마는 `prisma/schema.prisma` (520줄)에 있음.

### 설계 원칙
- 모든 번역 가능 엔티티는 `*Translation` 테이블 보유 — 새 언어 = 새 행, 스키마 변경 없음
- MVP에서는 제휴/외부 URL 필드 없음 — 수익화 전략 확정 후 추가
- 이미지 URL은 Cloudflare R2만 사용 — 외부 이미지 의존성 없음
- `hasBoard` 플래그: `Group`, `Artist`, `EventSeries`에 있음 — 관리자 제어, 게시판 무제한 생성 방지
...
| 테이블 | 역할 |
|---|---|
| `Group` | 플랫 태그 — 프랜차이즈, 레이블, 에이전시, 시리즈. `hasBoard` 관리자 제어. `category`: "anime"\|"kpop" 등 |
| `GroupTranslation` | 다국어 그룹 이름 |
| `ArtistGroup` | Artist ↔ Group N:N — 한 아티스트가 여러 그룹에 속할 수 있음 |

#### Artist 레이어
| 테이블 | 역할 |
|---|---|
| `Artist` | 공연 엔티티. `parentArtistId` 자기참조로 서브유닛 표현. `type`: "solo"\|"group"\|"unit"\|"band" |
| `ArtistTranslation` | 다국어 아티스트 이름 |

#### 공연자 레이어
| 테이블 | 역할 |
|---|---|
| `StageIdentity` | 팬이 공연자를 인식하는 이름/페르소나. `type`: "character"(애니)\|"persona"(K-POP). `color` 퍼스컬러 |
| `StageIdentityTranslation` | 다국어 캐릭터/멤버 이름 |
| `StageIdentityArtist` | N:N — 메구미 → 蓮ノ空, Mira-Cra Park!, KahoMegu♡Gelato |
| `RealPerson` | 실제 인간 공연자 — 애니는 성우, K-POP은 페르소나와 동일 |
| `RealPersonTranslation` | 다국어 실명 |
| `RealPersonStageIdentity` | 시간 인식 N:N — 성우 교체(`startDate`/`endDate`), 단기 커버(`note`) |

#### 이벤트 레이어
| 테이블 | 역할 |
|---|---|
| `EventSeries` | 관련 이벤트 묶음. `artistId` null이면 다수 아티스트 페스티벌. `parentSeriesId` 자기참조 |
| `EventSeriesTranslation` | 다국어 시리즈 이름 |
| `Event` | 하나의 공연 날짜 또는 레그 컨테이너. `status`: "upcoming"\|"ongoing"\|"completed"\|"cancelled" |
| `EventTranslation` | 다국어 이벤트 이름 |

#### 셋리스트 레이어
| 테이블 | 역할 |
|---|---|
| `SetlistItem` | 하나의 공연 슬롯. `stageType`: "full_group"\|"unit"\|"solo"\|"special". `status`: "rumoured"\|"live"\|"confirmed" |
| `SetlistItemSong` | N:N — 메들리 지원 (한 항목에 여러 곡). `order` 필드 |
| `SetlistItemMember` | 공연자 — 항상 `stageIdentityId`, 선택적 `realPersonId` |

#### 음악 레이어
| 테이블 | 역할 |
|---|---|
| `Song` | `artistId` 없음 — `SongArtist`로 크레딧. `baseVersionId` 자기참조로 버전 표현. `variantLabel`: "SAKURA Ver." 등 |
| `SongArtist` | N:N — 콜라보 지원. `role`: "primary"\|"featured"\|"cover" |
| `SongTranslation` | 다국어 곡 제목 |
| `Album` | `type`: "single"\|"album"\|"ep"\|"live_album"\|"soundtrack" |
| `AlbumTranslation` | 다국어 앨범 제목 |
| `AlbumTrack` | N:N — 한 곡이 여러 앨범에 수록 가능 |
...
| 테이블 | 역할 |
|---|---|
| `User` | `preferredLocale` 자동 번역 표시 기준. `contributionCount` 비정규화 캐시 |
| `Comment` | 아래 댓글 시스템 설계 참조 |
| `CommentTranslation` | `targetLocale`별 자동 번역 캐시 |
| `CommentLike` | 좋아요 junction |
| `CommentEdit` | 어펜드 전용 수정 이력 |
...
**서브유닛은 Artist 엔티티:**
Cerise Bouquet, DOLLCHESTRA, Mira-Cra Park! 모두 `parentArtistId → 蓮ノ空`인 `Artist` 행.

**곡 버전:**
"Dream Believers (SAKURA Ver.)" → `baseVersionId: "Dream Believers"`, `variantLabel: "SAKURA Ver."`.

**메들리:**
`SetlistItemSong` junction 테이블 + `order` 필드. `SetlistItem`에 직접 `songId` 없음.

**콜라보:**
`SongArtist` junction + `role`. "Link to the FUTURE" → SongArtist 행 3개.

**다수 아티스트 이벤트:**
이차원 페스 → `EventSeries`에 `artistId: null`, `organizerName: "Bandai Namco / Lantis"`.

**이벤트 레그 그룹화:**
`Event.parentEventId` 자기참조. "고베 Day 1" + "고베 Day 2" → 공통 `parentEventId: "고베 레그"`. 레그 컨테이너는 `date: null`.

**성우 교체:**
`RealPersonStageIdentity`의 `startDate`/`endDate`로 처리. `SetlistItemMember.realPersonId`는 항상 명시적.